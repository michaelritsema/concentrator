...
<module>
    <name>DNS Enforcement</name>
    <description>Detect and enforce DNS settings</description>
    <platforms>
    ...
    <parameters>
		...
		<parameter>
			...
			<displayName>Primary DNS Server</displayName>
			<description>Primary DNS IP Address</description>
			<type>string</type>
			<value>64.6.64.6</value>
			<required>true</required>
		</parameter>
		<parameter>
			<name>SecondaryDNS</name>
			<displayName>Secondary DNS Server</displayName>
			<description>Secondary DNS IP Address</description>
			<type>string</type>
			<value>64.6.65.6</value>
    ...
    <extension><![CDATA[<#
.SYNOPSIS
    Read the current DNS settings are enforce a specific primary and secondary
.DESCRIPTION
    This script retrieves the DNS settings for a specifically named interface
	and forces the value to specific IP addresses.
...
Write-Host "Primary DNS: $primaryIP"
Write-Host "Secondary DNS: $secondaryIP"
...
if ($mydns.Count -gt 4) {
	$mustOverwrite = $False
	$actualPrimary = $mydns[4]
	if ($actualPrimary -ne $primaryIP) {
		Write-Host "$actualPrimary is not $primaryIP"
		$mustOverwrite = $True
	}
	if ($secondaryIP.Length -gt 0) {
		if ($mydns.Count -gt 5) {
			$actualSecondary = $mydns[5]
			if ($secondaryIP -ne $actualSecondary) {
				Write-Host "$actualSecondary is not $secondaryIP"
				$mustOverwrite = $True
			}
		}
		else {
			Write-Host "$secondaryIP is specified for secondary, but no secondary was detected"
			$mustOverwrite = $True
		}
	}
	if ($mustOverwrite -eq $True) {
		"Must Overwrite - first emit the original dns settings"
		$resultSet = CreateResultSet
		AddRow $resultSet $mydns
		EmitExtensionResults $eventID $resultSet
		"Now setting DNS IP addresses - first primary at $primaryIP"
		netsh interface ip set dns name="$interfaceName" static $primaryIP primary
		if ($secondaryIP.Length -gt 0) {
			Write-Host "Adding secondary $secondaryIP"
			netsh interface ip add dns name="$interfaceName" $secondaryIP index=2
		}
	}
	else {
		Write-Host "No need to overwrite"
	}
}
...
# AgEAAgEAAgEAAgEAAgEAMCEwCQYFKw4DAhoFAAQUUP6GTNHSSSwJqtiXYVZnN6xz
# 9K2gghBcMIIE0zCCA7ugAwIBAgIQGNrRniZ96LtKIVjNzGs7SjANBgkqhkiG9w0B
...
# hvcNAQkEMRYEFOidQI+0ELe0OpXr9sXJjdVr1G0AMA0GCSqGSIb3DQEBAQUABIIB
# AGth6Ufsiwps1DuaVRZw7jFvSJFPHEI8+Fhu0/F9myxlP2fYQop/I5G+7V6u+kp7
# ojzRMGp9mP4wP4wIoegMUmAU0D8W3KhScOa38CH6ZIl3mogc9+3NLI4PI8yn5DBe
# pXeBdOWVuC+kU4JeoDMsMpRI0hy0AgnVzj/dlwFy6/i/UfGVnqAlwKvLEdtrgqzP
# BTtaeqGSi5Wssdspy9PVq3jrCw/6eGzbQIynlpOaayh6/bhHDGozSQuBwzRZDPkg
# kY3H+hJikPN80iaAtD5HQD8tckKmMcBZcYgwhFDX2c2vPXpC0fnQsV/xwXF+lZ7z
# YuLghwY5mT67RkVi2c0Eols=
