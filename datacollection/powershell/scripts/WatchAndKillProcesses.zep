...
<module>
    <name>Kill Named Process</name>
    <description>Scan and kill processes with specific name.</description>
    <platforms>
    ...
    <parameters>
      <parameter>
        <name>PollingTime</name>
        <displayName>Polling Time in seconds</displayName>
        <description>Number of seconds between process checks</description>
        <type>number</type>
        <value>60</value>
      ...
      <parameter>
			...
			<displayName>Process Names</displayName>
			<description>Name of process to kill (supports wildcards, but no extension)</description>
			<type>string</type>
			<value></value>
			<required>true</required>
		</parameter>
	...
	<!-- Output:
		eventID: 100201 
		tag0:	"notepad"				#Name of process terminated
		tag1:	"08/21/2015 10:32:05"	#Time process was started
	-->
...
.SYNOPSIS
    Runs and watches for any processes named (CSV list of names passed as parameter) and terminate them
.DESCRIPTION
    This script uses the Stop-Process command to kill all running processes that name matches
	one of a list passed into the script.
	
	Note at the top line the word PERSISTENT is immediately after the opening comment.
	This is required to specify it is a PERSISTENT script rather than a Ephemeral one.
.PARAMETER ProcessName[,ProcessName]
    [int]MaxFileSizeBeforeAlertSent - default is 1000, or 1GB
.NOTES
Name: WatchAndKillProcesses
Author: David Stidolph
DateCreated: 8/20/2015
...
Function KillMatchingProcesses([System.Collections.ArrayList]$resultSet, [string]$exename)
{
	"Trying to get processes named $exename"
...
if ( -not $gehret[0] )
{
	"we are not in 'persistent' mode - passed $pollTimeSeconds seconds and $processList"
	$resultSet = CreateResultSet
	$processList.split(" ") | foreach {KillMatchingProcesses $resultSet $_}
...
{
	"we are in persistent mode - passed $processList"
	...
	if ( $evtBegin.WaitOne() )
	{
		"Begin Event fired, now executing once each $pollTimeSeconds seconds until End Event fired"
		while (!$evtEnd.WaitOne($pollTimeSeconds*1000) )
		{	# Wait for time to expire unless end event fired
			$resultSet = CreateResultSet
			$processList.split(" ") | foreach {KillMatchingProcesses $resultSet $_}
			if ($resultSet.Count -gt 0) {
				EmitExtensionResults $eventID $resultSet
...
# AgEAAgEAAgEAAgEAAgEAMCEwCQYFKw4DAhoFAAQU48QwPBvZaRb/Ivfrt9ddsTtm
# KfigghBcMIIE0zCCA7ugAwIBAgIQGNrRniZ96LtKIVjNzGs7SjANBgkqhkiG9w0B
...
# hvcNAQkEMRYEFHEjErRXNC5uc+mvDwB6nPqqAHIdMA0GCSqGSIb3DQEBAQUABIIB
# AKiDzlBmcNoRkLCrNxptQzmHLE68yZJfpl4dr8UlzKqtPZMZH++waUxoyC2RTA0y
# P2HHBHmMMTEjBu0eo5O056RXbFrBTaxQf738n335HmD4A2GUW4X/BfDwk/mLC+5e
# f5TozZB57NLfv+Ef3LoxD0sDKPrF0Y1FMaK/lrYHT7AqXEz0eOBXYYVGEKM7+p8L
# tUtzOTr9iV5RtK2bEi1/KpWiMTV9UDYeuUuDVsUriWJjoDV4KnfDIIOhMvxpdJnb
# qMBfefrjU12HfD+/huOwZS5SVZdeMvyris/dH8Opb7B3hP9jrD9gigDWe5atfvdo
# UcT2TZQcjp1k6iRB6Cqrb+Q=
